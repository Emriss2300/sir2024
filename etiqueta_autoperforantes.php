...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $cantidadPorBolsa = isset($_POST['cantidad_por_bolsa']) ? mysqli_real_escape_string($conn, $_POST['cantidad_por_bolsa']) : '';
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    for ($i = 0; $i < $cantidadEtiquetas; $i++) {
                        ...
                        echo "<img src='/consulta/imagenes/2.png' alt='Imagen' class='ancho-imagen'>"; // Añade la clase 'ancho-imagen' aquí
                        echo "<h2 class='codigo' style='font-size: 16px; font-family: \"arial\", sans-serif;'>Código: {$row['codigo']}</h2>";
                        echo "<p class='descripcion' style='font-size: 14px;'> {$row['descripcion']}</p>";
                        echo "<p class='cantidad' style='font-size: 16px;'>Cantidad: $cantidadPorBolsa</p>";
